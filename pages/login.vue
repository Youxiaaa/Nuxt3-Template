...
<template>
  <section>
    <nuxt-link to="/" class="fixed top-20px left-20px w-40px h-40px flex items-center justify-center rounded-full border-4px border-black z-10">
      GO
    </nuxt-link>
    <div class="grid grid-cols-1 md:grid-cols-2">
      <div class="flex items-center justify-center min-h-screen after-content-none after-bg-[url(/images/login/login.svg)] after-bg-no-repeat after-bg-contain after-bg-center relative after-absolute after-top-0 after-left-0 after-w-full after-h-full after-opacity-50 md:after-opacity-0">
        <button class="border-4px border-black rounded-10px py-10px px-40px fw-800 text-20px bg-white fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 md:static z-10 md:-translate-x-0 md:-translate-y-0" @click="handleLogin">
          登入
        </button>
      </div>
